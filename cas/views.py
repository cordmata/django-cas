...
def proxy_callback(request):
    ...
    if not (pgtIou and tgt):
        logger.warning('No pgtIou or pgtId found in request.')
    ...
    try:
        ...
        logger.warning('PGT Ticket created.')
        ...
        return HttpResponse('PGT storage failed for tgt {tgt} and pgt {pgt}'.format(
            tgt=tgt,
            pgt=pgtIou
